...
//
// CoinDispenserPanel.java - The CoinDispenser panel of POStest
//
//------------------------------------------------------------------------------
...
public class CoinDispenserPanel extends Component implements StatusUpdateListener, ActionListener {
    
	private static final long serialVersionUID = 3091426102824935167L;

	protected MainButtonPanel mainButtonPanel;
    
    private CoinDispenser coinDispenser;
    
    private String defaultLogicalName = "defaultCoinDispenser";
    ...
    public void statusUpdateOccurred(StatusUpdateEvent sue) {
        System.out.println("CoinDispenser received status update event.");
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(ae.getActionCommand().equals("open")){
            try{
                ...
                coinDispenser.addStatusUpdateListener(this);
                
                coinDispenser.open(logicalName);
                deviceEnabledCB.setEnabled(false);
                freezeEventsCB.setEnabled(true);
                int version = coinDispenser.getDeviceServiceVersion();
                if(version >= 1009000) {
            ...
            try{
                ...
                msg += "\nCapPowerReporting: " + (coinDispenser.getCapPowerReporting() == JposConst.JPOS_PR_ADVANCED ? "Advanced" : (coinDispenser.getCapPowerReporting() == JposConst.JPOS_PR_STANDARD ? "Standard" : "None"));
                if(ver_18_complient)
                {
                	msg += "\nCapStatisticsReporting: " + coinDispenser.getCapStatisticsReporting();                    
                	msg += "\nCapUpdateStatistics: " + coinDispenser.getCapUpdateStatistics();
                }
                else
                {
            ...
            try{
                StatisticsDialog dlg = new StatisticsDialog(coinDispenser);
                dlg.setVisible(true);
            }catch(Exception e){
        ...
        try {
            deviceEnabledCB.setSelected(coinDispenser.getDeviceEnabled());
            freezeEventsCB.setSelected(coinDispenser.getFreezeEvents());
        } catch(JposException je) {
            System.err.println("CoinDispenserPanel: MethodListener: JposException");
        }
    }
            ...
            try {
                if (source == deviceEnabledCB){
                    if (e.getStateChange() == ItemEvent.DESELECTED){
                        coinDispenser.setDeviceEnabled(false);
                    }else{
                        coinDispenser.setDeviceEnabled(true);
                    }
                }else if (source == freezeEventsCB){
                    if (e.getStateChange() == ItemEvent.DESELECTED){
                        coinDispenser.setFreezeEvents(false);
                    }else{
                        coinDispenser.setFreezeEvents(true);
                    }
                }
            } catch(JposException je) {
                System.err.println("CoinDispenserPanel: CheckBoxListener: Jpos Exception" + e);
            }
        }
    }
    private class StatusTimerUpdateTask extends java.util.TimerTask{
        public void run(){
            if(coinDispenser != null){
                mainButtonPanel.currentStatus.setText(MainButtonPanel.getStatusString(coinDispenser.getState()));
            }
        }
    }
}
